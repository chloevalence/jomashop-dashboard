...
Generates an exportable PDF report for a given week with Valence branding.
Uses cache or S3 month caches for data.
"""
...
# Valence brand colors (from valence_monthly_report template)
...
# Logo paths: Colored (cover) and White (header bar)
...
def _make_on_page(subtitle: str):
    ...
    def on_page(cv, doc):
        ...
        cv.drawString(MARGIN, 8, f"Weekly QA Report  ·  {subtitle}  ·  Confidential  ·  Valence AI")
        ...
        for page in paginator.paginate(Bucket=config["bucket"], Prefix=prefix_full):
            for obj in page.get("Contents") or []:
                ...
                try:
                    ...
                    if isinstance(data, dict):
                        c = data.get(
                            "call_data", data.get("calls", data.get("data", []))
                        )
                    else:
...
def _prepare_dataframe(calls: List[Dict]) -> pd.DataFrame:
    ...
    if "Call Date" in df.columns:
        ...
        df["Call Date"] = pd.to_datetime(
            df["date_raw"], format="%m%d%Y", errors="coerce"
        )
...
def categorize_coaching(text: str) -> str:
    ...
    t = str(text).lower()
    scores = {
        "Product Knowledge": sum(
            1
            for kw in ["product", "knowledge", "unfamiliar", "specification"]
            if kw in t
        ),
        "Communication": sum(
            1
            for kw in ["communication", "clarity", "explain", "empathy", "tone"]
            if kw in t
        ),
        "Hold Time": sum(
            1 for kw in ["hold", "transfer", "escalate", "wait"] if kw in t
        ),
        "System/Technical": sum(
            1 for kw in ["system", "technical", "error", "software", "slow"] if kw in t
        ),
        "Process/Procedure": sum(
            1 for kw in ["process", "procedure", "policy", "workflow"] if kw in t
        ),
        "Efficiency": sum(
            1 for kw in ["efficient", "streamline", "reduce time", "faster"] if kw in t
        ),
    }
    m = max(scores.values())
...
def _build_agent_pages_reportlab(
    ...
    dur_col = (
        "Call Duration (min)" if "Call Duration (min)" in failing.columns else None
    )
    ...
    for agent_idx, agent in enumerate(all_agents):
        ...
        story.append(
            Paragraph(
                f"<b>{title}</b>", ParagraphStyle("Title", fontSize=14, spaceAfter=6)
            )
        )
        ...
        if len(agent_fails_df) > 0:
            ...
            for _, row in agent_fails_df.iterrows():
                ...
                handle = (
                    f"{dur:.1f}"
                    if isinstance(dur, (int, float)) and not pd.isna(dur)
                    else "—"
                )
...
def generate_report(
    week_df: pd.DataFrame, all_df: pd.DataFrame, week_start: datetime, output_path: Path
):
    """Generate the weekly report PDF with Valence branding."""
    ...
    id_col = "Call ID" if "Call ID" in week_df.columns else "call_id"
    ...
    story.append(Paragraph("Weekly QA Report", sTitleBig))
    story.append(Paragraph(week_label.replace("–", " – "), sSubtitle))
    story.append(Spacer(1, 0.12 * inch))
    story.append(HRFlowable(width=TW, thickness=2.5, color=V_PURPLE, spaceAfter=16))

    total_calls = int(week_df[id_col].nunique()) if len(week_df) > 0 and id_col in week_df.columns else 0
    avg_qa = week_df["QA Score"].mean() if len(week_df) > 0 and "QA Score" in week_df.columns else 0
    pass_rate = (week_df["QA Score"] >= PASS_THRESHOLD).mean() * 100 if len(week_df) > 0 and "QA Score" in week_df.columns else 0
    n_agents = week_df["Agent"].nunique() if len(week_df) > 0 and "Agent" in week_df.columns else 0
    kpis = [
    ...
    if len(week_df) > 0 and "Agent" in week_df.columns:
        ...
        id_c = call_col if call_col in week_df.columns else None
        if id_c:
        ...
        ax.set_xlim(40, 96)
        _style_ax(ax, title=f"Agent Leaderboard — Avg QA Score ({week_label})", xlabel="Avg QA Score")
    ...
    if len(all_df) > 0 and "Call Date" in all_df.columns:
        adf = all_df.copy()
        adf["Week"] = adf["Call Date"].dt.to_period("W-SUN").astype(str)
        weekly = adf.groupby("Week").agg(
            Pass_Rate=("QA Score", lambda s: (s >= PASS_THRESHOLD).mean() * 100),
            Avg_QA=("QA Score", "mean"),
        ).reset_index().sort_values("Week")
        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(7.4, 5.2), facecolor=P_LGRAY)
        x = range(len(weekly))
        ax1.plot(x, weekly["Pass_Rate"], color=P_BLUE, lw=2, marker="o", ms=5, zorder=3)
        ax1.fill_between(x, weekly["Pass_Rate"], alpha=0.12, color=P_BLUE)
        _style_ax(ax1, title="Pass Rate % — Weekly Trend", ylabel="Pass Rate %")
        ax1.set_xticks(list(x))
        ax1.set_xticklabels(weekly["Week"].tolist(), fontsize=7)
        ax1.grid(axis="y", color="white", lw=0.8)
        ax2.plot(x, weekly["Avg_QA"], color=P_GREEN, lw=2, marker="o", ms=5, zorder=3)
        ax2.fill_between(x, weekly["Avg_QA"], alpha=0.12, color=P_GREEN)
        ax2.axhline(70, color=P_AMBER, lw=1.2, ls="--", alpha=0.8)
        _style_ax(ax2, title="Avg QA Score — Weekly Trend", ylabel="Avg QA Score")
        ax2.set_xticks(list(x))
        ax2.set_xticklabels(weekly["Week"].tolist(), fontsize=7)
    ...
    if len(week_df) > 0 and "Hour" in week_df.columns and "QA Score" in week_df.columns:
        by_hour = week_df[week_df["Hour"].notna()]
        if len(by_hour) > 0:
    ...
    for agent in all_agents:
        ...
        if len(agent_fails_df) == 0 and not top_coaching:
            continue
        agent_week = week_df[week_df["Agent"] == agent] if len(week_df) > 0 else pd.DataFrame()
        if len(agent_week) > 0:
            agent_avg = agent_week["QA Score"].mean()
            agent_pass = (agent_week["QA Score"] >= PASS_THRESHOLD).mean() * 100
            agent_calls = int(agent_week[id_col].nunique()) if id_col in agent_week.columns else len(agent_week)
        else:
            agent_avg, agent_pass, agent_calls = 0, 0, 0
        n_failed = len(agent_fails_df)

        story.append(Paragraph(f"{agent} — Coaching & Failing Calls", sH1))
        story.append(HRFlowable(width=TW, thickness=1.5, color=V_PURPLE, spaceAfter=6))
        pill_cells = [[
            Paragraph(f'<font size="15" color="#4900A7"><b>{agent_avg:.1f}</b></font><br/><font size="7" color="#64748B">Avg QA Score</font>', sCenter),
            Paragraph(f'<font size="15" color="#4900A7"><b>{agent_pass:.1f}%</b></font><br/><font size="7" color="#64748B">Pass Rate</font>', sCenter),
            Paragraph(f'<font size="15" color="#4900A7"><b>{agent_calls}</b></font><br/><font size="7" color="#64748B">Total Calls</font>', sCenter),
            Paragraph(f'<font size="15" color="#4900A7"><b>{n_failed}</b></font><br/><font size="7" color="#64748B">Failed Calls</font>', sCenter),
        ]]
        pill_tbl = Table(pill_cells, colWidths=[TW / 4] * 4)
        ...
        if top_coaching:
            story.append(Paragraph("Top Rubric Failures", sBold))
            for rank, (code, count) in enumerate(top_coaching, 1):
                label = rubric_labels.get(str(code).strip(), code)
                lab = str(label).replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
                story.append(Paragraph(f'<font color="#4900A7"><b>{rank}.</b></font> {lab}: <b>{count}</b>', sCoach))
            story.append(Spacer(1, 7))
        ...
        if len(agent_fails_df) > 0:
            ...
            for i, s in enumerate(score_ints):
                if s < 50 and s > 0:
                    call_ts += [("BACKGROUND", (2, i + 1), (2, i + 1), colors.HexColor("#FEE2E2")), ("TEXTCOLOR", (2, i + 1), (2, i + 1), colors.HexColor("#991B1B"))]
                elif s < 70 and s > 0:
                    call_ts += [("BACKGROUND", (2, i + 1), (2, i + 1), colors.HexColor("#FEF3C7")), ("TEXTCOLOR", (2, i + 1), (2, i + 1), colors.HexColor("#92400E"))]
                elif s > 80:
                    call_ts += [("BACKGROUND", (2, i + 1), (2, i + 1), colors.HexColor("#D1FAE5")), ("TEXTCOLOR", (2, i + 1), (2, i + 1), colors.HexColor("#065F46"))]
            call_tbl.setStyle(TableStyle(call_ts))
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Week start in YYYY-MM-DD format (e.g. 2026-03-01 for first week of March)",
    )
    parser.add_argument(
        "--output",
        type=str,
        default=None,
        help="Output PDF path (default: weekly_report_YYYY-MM-DD.pdf)",
    )
    parser.add_argument(
        "--cache",
        type=str,
        default=str(CACHE_FILE),
        help="Path to cache file (for disk cache)",
    )
    ...
    if (year, month) not in MONTHS_AVAILABLE:
        print(
            f"⚠️  Week {args.week} may not have cached data. Available months: {MONTHS_AVAILABLE}"
        )

    output_path = (
        Path(args.output)
        if args.output
        else PROJECT_ROOT / f"weekly_report_{args.week}.pdf"
    )
    ...
    if not all_calls:
        ...
        if not all_calls:
            print(
                "❌ No data found. Ensure cache exists or S3 is configured (.streamlit/secrets.toml)"
            )
            sys.exit(1)
    ...
    week_end = week_start + timedelta(days=6)
    week_mask = (df["Call Date"] >= pd.Timestamp(week_start)) & (
        df["Call Date"] <= pd.Timestamp(week_end)
    )
    ...
    if len(week_df) == 0:
        ...
        if month_calls:
            ...
            if len(month_prep) > 0:
                ...
                week_df = df[week_mask].copy()
                print(
                    f"   Supplemented with {len(week_df)} calls for week of {args.week} from S3"
                )
