...
def deduplicate_calls(call_data):
    ...
    for idx, call in enumerate(call_data):
        ...
        # Send heartbeat every 1000 calls or every 2 seconds during deduplication
        if (idx + 1) % 1000 == 0 or time.time() - last_heartbeat_time >= 2:
            try:
                send_heartbeat()
            except Exception:
                pass  # Heartbeat might not be available in all contexts
            last_heartbeat_time = time.time()
    ...
    try:
        ...
        try:
            ...
            if use_streamlit_cache:
                ...
                # Deduplication deferred - will be done when user clicks "Load Data"
                # Skip deduplication here to prevent crash after S3 load
                # Deduplication will happen after lazy_load_data check
...
if (
    ...
    and st.session_state["authenticator"] is not None
):
    # Use cached authenticator - already initialized
    authenticator = st.session_state["authenticator"]
    logger.debug("Using cached authenticator from session_state")
else:
...
if skip_auth_after_failures:
    ...
    try:
        ...
        pass  # Ignore if session_state not available
else:
    for attempt in range(max_retries):
        try:
    ...
    if authenticator is None:
        ...
        with col1:
            if st.button(
                ...
                # Clear cached authenticator to force reinitialization
...
if is_super_admin():
    ...
    if refresh_in_progress:
        ...
        try:
            ...
            if refresh_start_time_str:
                ...
                elapsed_minutes = (
                    datetime.now() - refresh_start_time
                ).total_seconds() / 60
    ...
    if refresh_in_progress:
        ...
        try:
            log_audit_event(
                current_username, "refresh_data", "Refreshed new data from S3"
            )
        except Exception as audit_error:
            logger.warning(
                f" Could not log audit event: {audit_error} - continuing anyway"
            )
        ...
        try:
            ...
            st.error(
                " Refresh failed: could not initialize refresh state. Please try again."
            )
            st.stop()
        ...
        try:
            ...
            logger.debug(
                f" Calling load_all_calls_cached with cache_version={cache_version}"
            )
            streamlit_result = load_all_calls_cached(cache_version=cache_version)
            logger.debug(
                f" load_all_calls_cached returned: type={type(streamlit_result)}"
            )
            ...
            previous_streamlit_errors = (
                streamlit_result[1]
                if streamlit_result
                and len(streamlit_result) > 1
                and streamlit_result[1]
                else []
            )
            logger.info(
        ...
        if isinstance(new_errors, str):
            ...
            # Check if there are remaining files to process before marking as complete
            remaining_files = 0
            total_s3_files = 0
            processed_files = 0
            try:
                # Get file counts from session state (set by load_new_calls_only)
                ...
                if total_s3_files == 0 or processed_files == 0:
                    ...
                    try:
                        disk_cache_metadata = load_cached_data_from_disk()
                        if disk_cache_metadata and len(disk_cache_metadata) > 0:
                            # Check if cache has partial metadata
                            # This is a fallback - session state should have the info
                            pass
                    except Exception:
                        pass
...
if call_data and isinstance(call_data, list) and len(call_data) > 0:
    # Show progress indicator for deduplication
    dedup_progress = st.progress(0)
    dedup_status = st.status("Deduplicating calls...", expanded=False)

    send_heartbeat(force=True)
    original_count = len(call_data)
    dedup_progress.progress(0.2)

    # Perform deduplication with heartbeats
    # The deduplicate_calls function already has optimizations, but we'll add heartbeats
    # by calling it and monitoring progress
    logger.info(f"Starting deduplication of {original_count} calls...")

    # Call deduplicate_calls - it will check for duplicates efficiently
    call_data = deduplicate_calls(call_data)
    dedup_progress.progress(0.8)
    send_heartbeat()

    final_count = len(call_data)
    if final_count < original_count:
        logger.info(
            f"Deduplicated calls: {original_count} → {final_count} unique calls (removed {original_count - final_count} duplicates)"
        )
    else:
        logger.info(f"No duplicates found - all {final_count} calls are unique")

    dedup_progress.progress(1.0)
    dedup_status.update(label="Deduplication complete", state="complete")
    dedup_progress.empty()
    dedup_status = None
    send_heartbeat()
                ...
                if "Reason" in filtered_df.columns:
                    ...
                    with reason_col1:
                        ...
                        if len(reason_counts) > 0:
                            ...
                            reason_counts.plot(
                                kind="barh", ax=ax_reason, color="steelblue"
                            )
                    ...
                    if "Call Date" in filtered_df.columns and len(filtered_df) > 0:
                        ...
                        filtered_df_reason = filtered_df_reason.dropna(
                            subset=["Call Date"]
                        )
                ...
                if "Outcome" in filtered_df.columns:
                    ...
                    with outcome_col1:
                        ...
                        if len(outcome_counts) > 0:
                            ...
                            outcome_counts.plot(
                                kind="barh", ax=ax_outcome, color="green"
                            )
        ...
        if all_coaching:
            ...
            if selected_insight == "Most Common Coaching Suggestions":
                ...
                category_df["Percentage"] = (
                    (category_df["Frequency"] / len(all_coaching) * 100).round(1)
                    if all_coaching
                    else pd.Series([0.0] * len(category_df))
                )
                ...
                all_coaching_df["Percentage"] = (
                    (all_coaching_df["Frequency"] / len(all_coaching) * 100).round(1)
                    if all_coaching
                    else pd.Series([0.0] * len(all_coaching_df))
                )
                st.write(
